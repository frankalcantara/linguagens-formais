...
**3.** Se ∣q_1∣=3 e ∣q_2∣=4, quantos estados o autômato produto M=$M_1$cup$M_2$ terá?
...
**Exemplo Adicional: Interseção de "Começa com 'a'" com "Número Par de 'b's"**

Vamos construir um exemplo mais elaborado para solidificar o entendimento da construção de interseção.
...
1. Adicionar um **estado de erro** (ou **estado sumidouro**) $q_{erro}$
2. Para toda transição indefinida $\delta(q, a)$, definir $\delta(q, a) = q_{erro}$
3. Definir $\delta(q_{erro}, a) = q_{erro}$ para todo $a \in \Sigma$
4. Garantir que $q_{erro} \notin F$ (o estado de erro não é final)
...
Adicionamos um estado de erro $q_{erro}$ e completamos as transições:

- **Estados**: $Q' = \{q_0, q_1, q_2, q_{erro}\}$
- **Transições completadas**:
  - $\delta(q_2, b) = q_{erro}$ (transição faltante)
  - $\delta(q_{erro}, a) = q_{erro}$, $\delta(q_{erro}, b) = q_{erro}$ (auto-loops no erro)
...
| $q_1$ | $q_1$ | $q_2$ |
| $*q_2$ | $q_1$ | $q_{erro}$ |
| $q_{erro}$ | $q_{erro}$ | $q_{erro}$ |

**Passo 2: Construir o Complemento**

$F' = Q' - F = \{q_0, q_1, q_2, q_{erro}\} - \{q_2\} = \{q_0, q_1, q_{erro}\}$
...
- **String "abb"**: $q_0 \xrightarrow{a} q_1 \xrightarrow{b} q_2 \xrightarrow{b} q_{erro}$ ✓ (original rejeita, complemento aceita)
