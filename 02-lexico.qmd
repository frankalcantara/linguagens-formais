...
**1.** Dado o AFD M=(s\_0,s\_1,s\_2,s\_3,a,b,delta,s\_0,s\_0,s\_3), identifique cada um dos cinco componentes da tupla com sua respectiva descrição.

**2.** Um modelo foi definido como M=(Q,Sigma,delta,q\_0,F) onde Q=q\_0,q\_1, Sigma=0,1, q\_0=q\_0, e F=q\_2. Este modelo representa um AFD válido? Justifique sua resposta com base nas restrições formais.

**3.** Se um Autômato Finito Determinístico possui ∣Q∣=3 estados e um alfabeto com ∣Sigma∣=4 símbolos, qual é o número exato de pares no domínio da função de transição delta?
...
**5.** É possível que o estado inicial q\_0 também seja um estado de aceitação, ou seja, q\_0inF? Se sim, o que isso implica sobre a aceitação da *string* vazia, epsilon?
...
**1.** Considere um AFD que reconhece identificadores que começam com 'l' (letra) e são seguidos por 'd' (dígito). O alfabeto é Sigma=l,d. As transições definidas são delta(q\_0,l)=q\_1 e delta(q\_1,d)=q\_1. O estado de aceitação é F=q\_1. Este autômato é **completo**? Se não, adicione um estado de erro q\_e e liste todas as transições que faltam para completá-lo.
...
**4.** Se um AFD completo não possui um estado de erro explicitamente desenhado em seu diagrama, o que se assume sobre as transições não mostradas?

**5.** Dada a definição delta:QtimesSigmarightarrowQ, se a máquina está no estado q\_i e lê um símbolo ainSigma, é possível que ela permaneça no mesmo estado, ou seja, delta(q\_i,a)=q\_i? Dê um exemplo prático.
...
A @tbl-Autômato Finito Determinístico1 ilustra um exemplo de tabela de transições para um Autômato Finito Determinístico:
...
: Exemplo de tabela de transições para um Autômato Finito Determinístico que aceita _strings_ que contenham pelo menos um '0' seguido, em algum momento, por pelo menos um '1'. {#tbl-Autômato Finito Determinístico1}

Na @tbl-Autômato Finito Determinístico1, as convenções notacionais seguem o padrão estabelecido na literatura de teoria da computação:
...
A @fig-Autômato Finito Determinístico1 apresenta o diagrama de transições correspondente à tabela @tbl-Autômato Finito Determinístico1:

::: {#fig-Autômato Finito Determinístico1}
![](images/dia1.webp)
...
No diagrama da @fig-Autômato Finito Determinístico1, as convenções visuais estabelecem uma linguagem gráfica precisa:
...
Para o Autômato Finito Determinístico ilustrado na @tbl-Autômato Finito Determinístico1 e na @fig-Autômato Finito Determinístico, um autômato que aceita _strings_ contendo um '0' seguido em algum momento por um '1' será dada pela 5-tupla $M = (Q, \Sigma, \delta, q_0, F)$ definida como:
...
**1.** Desenhe o diagrama de transições para o AFD descrito pela seguinte tabela. Use as convenções gráficas para o estado inicial e os estados de aceitação.
...
**1.** Projete o diagrama de transições de um AFD sobre Sigma=a,b que aceite todas e somente as *strings* que contêm a substring "aba".

**2.** Projete o diagrama de transições de um AFD sobre Sigma=0,1 que aceite *strings* que representam números binários cujo valor é múltiplo de 3\. (Dica: Mantenha o resto da divisão por 3 como estado. v(w0)=2cdotv(w) e v(w1)=2cdotv(w)+1).

**3.** Projete a tabela de transições de um AFD que reconhece comentários de uma linha em uma linguagem de programação hipotética. Um comentário começa com // e vai até o final da linha (não nos preocuparemos com o final da linha). O alfabeto é Sigma=/,c, onde 'c' representa qualquer outro caractere.

**4.** Desenhe o diagrama de um AFD que aceite *strings* em a,b que tenham comprimento ímpar e terminem com 'a'.

**5.** Projete um AFD que aceite apenas as *strings* cat e car sobre o alfabeto Sigma=c,a,t,r.
...
**1.** Considere dois AFDs sobre Sigma=a,b: M\_1 aceita *strings* com número par de 'a's. M\_2 aceita *strings* que terminam com 'b'. Usando a construção do produto, construa a tabela de transições do autômato M=M\_1cupM\_2.

**2.** No autômato produto M do exercício anterior, quais são o estado inicial e o conjunto de estados de aceitação?

**3.** Se ∣Q\_1∣=3 e ∣Q\_2∣=4, quantos estados o autômato produto M=M\_1cupM\_2 terá?

**4.** Usando a construção do produto, o que aconteceria se tentássemos unir um autômato consigo mesmo (M\_1cupM\_1)? O autômato resultante seria diferente do original?

**5.** A construção do produto para a união sempre resulta em um AFD completo se os autômatos originais M\_1 e M\_2 forem completos? Justifique

#### Fechamento sob Interseção

**Teorema (Fechamento sob Interseção)**: Sejam $M_1$ e $M_2$ dois Autômatos Finitos Determinísticos que reconhecem $L_1$ e $L_2$. Então existe um Autômato Finito Determinístico $M$ que reconhece $L_1 \cap L_2$.

A construção é similar à união, mas com uma modificação crucial no conjunto de estados finais:

- $F = F_1 \times F_2$ (aceita apenas se **ambos** os autômatos aceitam)

Esta diferença sutil tem um impacto profundo: enquanto a união implementa uma lógica "OU", a interseção implementa uma lógica "E". A atenta leitora notará que esta construção permite expressar condições complexas através da composição de autômatos simples.
...
A construção do complemento é surpreendentemente elegante:

Dado $M = (Q, \Sigma, \delta, q_0, F)$, construímos $M' = (Q, \Sigma, \delta, q_0, F')$ onde:
- $F' = Q - F$ (os estados de aceitação tornam-se estados de rejeição e vice-versa)

**Observação Crítica**: Esta construção só funciona para Autômatos Finitos Determinísticos **completos**, onde toda transição está definida. Para Autômatos Finitos Determinísticos incompletos, primeiro devemos adicionar um estado de erro e completar as transições faltantes.

**Exemplo Ilustrativo**: Se $M$ aceita _strings_ com número par de '1's, então $M'$ (com estados finais invertidos) aceita _strings_ com número ímpar de '1's.

#### Exercícios 7 {#exercicio-7}

**1.** Usando os mesmos autômatos M\_1 (par de 'a's) e M\_2 (termina com 'b') do exercício 6.1, construa a tabela de transições para o autômato que reconhece L(M\_1)capL(M\_2).

**2.** Qual é o conjunto de estados de aceitação para o autômato de interseção do exercício 7.1?

**3.** Considere o AFD M que aceita strings sobre Sigma=a,b que contêm pelo menos um 'a'.  
M=(q\_0,q\_1,a,b,delta,q\_0,q\_1) com delta(q\_0,a)=q\_1, delta(q\_0,b)=q\_0, delta(q\_1,a)=q\_1, delta(q\_1,b)=q\_1.  
Construa o autômato M′ que reconhece a linguagem complemento overlineL(M). Qual linguagem M′ reconhece?  

**4.** Para construir o complemento de um AFD, o texto afirma que ele deve ser **completo**. Por que essa condição é necessária? O que poderia dar errado se aplicássemos a troca de estados finais em um autômato incompleto?
