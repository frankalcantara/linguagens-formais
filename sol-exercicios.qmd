...
Então:
- $w \in L(M_1 \cup M_2)$ (pois $w \in L(M_1)$)
- $w \notin L(M_1 \cap M_2)$ (pois $w \notin L(M_2)$)
...
- Isso é equivalente a dizer que $M_2$ aceita $w$, pois a componente $q$ segue exatamente as transições de $M_2$
...
Quando $M_1$ aceita tudo ($L(M_1) = \Sigma^*$), a interseção $M_1 \cap M_2$ se comporta exatamente como $M_2$, pois a única restrição vem de $M_2$.
...
- **$C_2$**: Para strings em $C_2$, qualquer sufixo mantém a string em $L$ (pois já contém $00$)
...
Por exemplo, o estado $(q_{1,int}, q_{d,id}, q_{d,op})$ seria um estado final em $A_{comb}$ porque $q_{1,int}$ é um estado final em $A_{int}$. Da mesma forma, um estado como $(q_{d,int}, q_{d,id}, q_{2,op})$ também seria final, pois $q_{2,op}$ pertence a $F_{op}$.
